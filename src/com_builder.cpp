...
BOOL CMDRlist::Execute(const std::string &verb, Player* mobile,std::vector<std::string> &args,int subcmd)
{
  ...
  world->WriteLog("sent number of rooms");
  itEnd=nums.end();
...
BOOL CMDZcreate::Execute(const std::string &verb, Player* mobile,std::vector<std::string> &args,int subcmd)
{
  ...
  if (args.size() != 3 || !min || !max)
    {
      mobile->Message(MSG_ERROR,"Syntax: zcreate <name of zone> <min> <max> creates a new zone with the name and rooms aviable in the range from min to max\n");
      return false;
    }

  if(min >= max)
    {
      mobile->Message(MSG_ERROR, "Max value must be greater than min value.\n");
      return false;
    }
